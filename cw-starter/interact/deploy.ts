...
async function instantiate(codeID: number): Promise<InstantiateResult> {
    ...
    console.log(senderAddress)
    const msg = {
       admin: "orai1dfpf94hmhrrmuu4mvx4wcce2yv3rqe8xhtnmg9"
...
async function CreatePoll() {
    ...
    const msg = {
        create_poll: {
            poll_id: process.env.CONTRACT_NAME,
            question:"Dam ba cai rust de vl",
            options:[("juno",0),("osmosis",1)],
    },
}
    console.log("msg", msg);
    const fee = "auto"
...
async function main() {
    ...
    writeToEnvFile("CONTRACT_NAME1", resInitiate.contractAddress)
